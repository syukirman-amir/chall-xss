...
function handleSubmit(event) {
    ...
    for (let link of links) {
        ...
        if (!dataType || (decodedDataType !== 'link' && decodedDataType !== 'link\n')) {
            outputDiv.innerHTML = '<p style="color:red;">Error: Only &lt;a&gt; tags with \'data-type=link\' and \'data-value=\' are allowed!</p>';
            return;
        }
    ...
    elements.forEach(node => {
        if (node.nodeType === Node.ELEMENT_NODE) {
            if (node.tagName.toLowerCase() !== 'a') {
                ...
                // Pastikan tag <a> memiliki data-type yang valid
                const dataType = node.getAttribute('data-type');
                const decodedDataType = decodeURIComponent(dataType || '');
                if (!dataType || (decodedDataType !== 'link' && decodedDataType !== 'link\n')) {
                    isValid = false; // Tolak jika data-type tidak valid
                }
    ...
    // Proses semua elemen <a> dengan data-type yang valid
